                  ...
                  <CardContent className="p-3">
                    {threads.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Messages will appear here when you accept applicants</p>
                      </div>
                    ) : (
                        ...
                        {threads.map(thread => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full text-left p-3 rounded-xl transition-colors ${selectedThreadId === thread.id ? "bg-ocean-50" : "hover:bg-gray-50"}`}
                            >
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-ocean-100 flex items-center justify-center text-xs font-semibold text-ocean-700 shrink-0">
                                  {other.initial}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between">
                                    <p className="text-sm font-medium text-gray-900 truncate">{other.name}</p>
                                    {unread > 0 && <span className="w-2 h-2 rounded-full bg-ocean-500" />}
                                  </div>
                          ...
                          {messages.map(msg => {
                            ...
                            return (
                              <div key={msg.id} className={`flex ${isMine ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[80%] p-3 rounded-2xl ${isMine ? "bg-ocean-600 text-white" : "bg-gray-100 text-gray-900"}`}>
                                  {msg.isSystem ? (
                                    <p className="text-xs italic">{msg.content}</p>
                                  ) : (
                                    <>
                                      <p className="text-sm">{msg.content}</p>
                                      <p className={`text-xs mt-1 ${isMine ? "text-ocean-200" : "text-gray-400"}`}>{timeAgo(msg.createdAt)}</p>
                                    </>
                                  )}
                                </div>
                              </div>
